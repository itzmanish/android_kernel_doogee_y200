...
#define CUST_EINT_HOME_TYPE 8
#define CUST_EINT_HOME_DEBOUNCE_EN 0

#define VANZO_HOME_KEY_USE_EINT
#ifdef VANZO_HOME_KEY_USE_EINT
#define KPD_PWRKEY_EINT 120
#define GPIO_HOME_EINT_PIN 120
...
void kpd_homekey_handler(unsigned long data)
{
  ...
  //if (kpd_show_hw_keycode) {
    printk("SUSLOV_DEBUG (%s) HW keycode = using EINT\n",
...
{
  mt_set_gpio_mode(GPIO_HOME_EINT_PIN, 0);
  mt_set_gpio_dir(GPIO_HOME_EINT_PIN, 0);
  mt_set_gpio_pull_enable(GPIO_HOME_EINT_PIN, 1);
  mt_set_gpio_pull_select(GPIO_HOME_EINT_PIN, 1);
